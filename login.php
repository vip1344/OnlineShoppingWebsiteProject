...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
    header("location: dsasaddsdsadsdsdasadsdssda.php");
    exit;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($email_err) && empty($password1_err)){
        ...
        if($stmt = mysqli_prepare($con, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id,$first_name, $email, $password);
                    if(mysqli_stmt_fetch($stmt)){
                        if(($password1== $password)){
                            ...
                            $_SESSION["name_"]=$first_name;
                            // Redirect user to welcome page
                            header("location: shop.php");
                            
                        } else{
                            // Display an error message if password is not valid
                            $password1_err = "The password you entered was not valid.";
                        }
                    }
                } else{
